...
The original Q4 MVP state model was per-Task private state stored in local WAL on the owning Node, requiring state serialization and BRP-channel transfer on Migrating. The user pushed for stronger semantics: cluster-wide accessibility for consistency and transaction support. We resolve this by giving each Task **private ownership** of its state, but storing that state in a **cluster-shared KV** that runs as a second logical state machine on the same Raft cluster as the Control Plane. Each Node is simultaneously a Worker, a Raft participant, and a KV node. The KV API is `get / put / cas / txn(ops)` over opaque bincode values — no range scan, no secondary index in MVP. This simplifies Migrating (new owner Node reads the latest checkpoint from KV in one Raft read), simplifies Failover (new owner reads checkpoint from KV + replays upstream from the saved offset), and provides linearizable reads/writes plus multi-key transactions for atomic state-and-offset snapshots.
