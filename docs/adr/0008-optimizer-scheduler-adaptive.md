...
The user's original design mentioned "scheduling strategy" and "optimizer" in the same breath. We separate them into two layers and add a third:
...
- **Scheduler** (`bee-control`): Cross-Node Task placement and rebalancing. MVP: simple bin-packing (resource-declaration vs Node capacity) + Work-Stealing. 0.7: feedback-driven rebalancing using per-Phase metrics (the user's "optimize scheduling by per-Phase time" requirement).
...
- The user's "optimize scheduling strategy by per-Phase time" requirement is satisfied by MLFQ default: short-Phases get higher priority, long-Phases get demoted, starving Phases get promoted. SJF/SRTN give strict shortest-first semantics; HRRN balances short jobs and long-waiting jobs.
