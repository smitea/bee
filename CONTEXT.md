...
- **Tests**: 146 Rust tests pass in `app/src-tauri` (+30 vs prior 116 — tenant module, cluster_profiles repo, application tenant column); 104 Vitest tests pass in `app/` (+9 vs prior 95 — SettingsModal tenant section, DatasourcesPage plugins badge, tenantStore, applicationsStore tenant path).
- **Bee Client (`app/`)** — Tauri 2.x + React + Vite + TypeScript + Tailwind; `productName: Bee Client`, identifier `io.smitea.beeclient`:
  ...
  - **Tenant enforcement (1.x item, ADR-0010)** — `tenant.rs` module owns `TenantAccess`, `can_access_datasource(job_tenant, ds_tenant)` (true when `ds_tenant == 0 || ds_tenant == job_tenant`) and `validate_tenant(value)` (clamps to 0..=65535). New SQLite migration v9 adds `applications.tenant INTEGER NOT NULL DEFAULT 0`. New Tauri commands `tenant_get` / `tenant_set(value)` read/write `client_settings["tenant"]`; `application_create` now accepts an optional tenant and defaults to the active one. Settings modal grows a Tenant section with debounced save + range validation. NavTree Application creation requires tenant (falls back to active tenant when blank). **1.x item resolved.**
  - **Multi-cluster saved connections (1.x item)** — New SQLite migration v10 introduces `cluster_profiles` table (`id PK, label, addr UNIQUE, tenant INTEGER NOT NULL DEFAULT 0, last_used_at, created_at`) making saved connections first-class instead of localStorage. New `db/clusters.rs` repo: `list`, `save`, `remove`, `set_active`, `get_active`. New Tauri commands `cluster_profile_list`, `cluster_profile_save`, `cluster_profile_remove`, `cluster_profile_activate(addr)` (parses addr, calls `connection::ensure_bundle` to switch live connection, persists addr to `client_settings["addr"]`, records audit). New `cluster_profile_migrate_legacy` accepts the legacy `bee-gui.connections` JSON array on first run and inserts each as a row, then clears the localStorage key. New sidebar component `ClusterProfilesSidebar` in `NavTree` selects / removes / edits each profile. **1.x item resolved.**
  - **Plugin Registry real integration (C)** — `plugin_registry.rs` already consults any loaded `bee_plugin_sdk` plugin via `load()`; empty registry returns empty summaries. DatasourcesPage header now shows a `Plugins registered: N` badge reflecting the live count. **Resolved.**
