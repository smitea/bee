...
## Current state (snapshot — 2026-07-30, final)
...
- **Tests**: **193 Rust tests pass in `app/src-tauri`** (+47 vs prior 146 — topology multi-node, encrypted Application I/O round trips, plugin loader, rolling restart orchestration, integration tests); **175 Vitest tests pass in `app/`** (+71 vs prior 104 — dashboard editor, topology graph, DAG designer, dynamic SchemaForm, search debounce/cancellation, plugin reload, klinecharts panel, settings merge).
- **Bee Client (`app/`)** — Tauri 2.x + React + Vite + TypeScript + Tailwind; `productName: Bee Client`, identifier `io.smitea.beeclient`. **All work pushed to `origin/main`; HEAD is `c094cfd`.** Eight development slices delivered end-to-end:
  1. **Foundation (slice 1)** — single-row top header (Bee brand + cluster dropdown + refresh + settings + theme), left NavTree (Cluster, global search, Applications count + add, expandable Application tree, refresh icons per section), right page-tab workspace (deduplicated closable tabs, pin / close / close-others / close-to-right / session restoration), fixed bottom activity/connection bar (red/green/pulsing indicator + latest audit event link).
  2. **Persistence + typed IPC (slice 2)** — **14 forward-only SQLite migrations** (`app/src-tauri/src/db/mod.rs`), repository modules (`applications`, `dashboards`, `settings`, `tabs`, `profiles`, `clusters`, `audit`, etc.), TanStack Query on the frontend, Zustand for presentation only, typed IPC wrappers under `app/src/ipc/`. **Components never touch `localStorage` or SQLite directly.**
  3. **Settings (slice 3)** — two-column modal with **11 categories** (Client, Connection, Appearance, Logging, Diagnostics, Cluster, Raft, KV, Scheduling, Plugins, Security). All auto-save with `Saving` / `Saved` / `Error` states; Connection exposes only **Test Connection** and **Connect** (no Save button). Cluster settings use versioned config + quorum-safe rolling restart.
  4. **Application lifecycle (slice 4)** — `application_enable` is a real orchestrator: builds a `RehydrationPlan` from local pipeline definitions + datasources, executes `AdminRequest::Deploy` and `AdminRequest::RegisterDatasource`, writes per-resource audit events plus a summary outcome (`Success` / `Degraded` / `Failure`). `application_disable` captures a snapshot. Pure helpers (`build_rehydration_plan`, `execute_rehydration`) backed by a `DeployRegistrar` trait so the whole path is unit-tested with a mock.
  5. **Search + Pipelines + Live Dashboard (slice 5)** — global text-change search debounced with stale-request cancellation, merges local Application/Dashboard results with AdminServer cluster-resource search by relevance; Pipeline editor uses **CodeMirror 6** (`@codemirror/lang-sql` + `@uiw/react-codemirror`); Application Dashboard is a configurable grid (add / drag / resize / remove panels: K-line, Active jobs, Tasks/sec, CPU, Pipeline status, Audit feed, Cluster topology) backed by SQLite.
  6. **Rolling restart + Plugins + Datasource SchemaForm (slice 6)** — quorum-safe rolling restart orchestration (`rolling_restart.rs`), live Plugin Registry loader (`plugin_registry.rs`) wired into the DatasourcesPage header (`Plugins registered: N` badge), schema-driven **SchemaForm** generated from each Plugin's Adapter configuration schema, Test Connection + Connect and Save. Encrypted Application import/export (argon2 KDF + AES-GCM). 1.x items resolved: tenant enforcement (`tenant.rs`, migration v9) and multi-cluster saved connections (migration v10 `cluster_profiles`, `db/clusters.rs` repo, `cluster_profile_*` commands).
  7. **CSP hardening + lifecycle accuracy + visual verification (slice 7)** — restrictive production CSP, `core:default`-only capabilities, RehydrationPlan mock-tested path, `cargo tauri dev` verified on macOS M2 (screenshots at `/tmp/bee-client-1.png` and `/tmp/bee-client-2.png`).
  8. **Topology + DAG designer + Plugin reload + K-line chart + Docker (slice 8)** — **`@xyflow/react` cluster topology graph** (symmetric ring, leader chassis centered, orthogonal edges, runtime status overlay), **DAG designer** for Pipelines (Input Datasource → Phase Handler(s) → Output Datasource, cross-Pipeline edges distinguished from in-process edges), **`klinecharts` K-Line panel** integration, **Plugin `Reload from disk`** button (re-scans the plugin directory and re-loads via `load()`), ECharts trend panels, end-to-end **Bee cluster integration tests** against the Rust harness, **5-node Docker cluster** (`docker/docker-compose.yml`) + plugin deploy script (`scripts/deploy-plugins.sh`), Bee Client image recipe (`docker/Dockerfile.bee-client`), and a **sample K-Line plugin** (`examples/kline_plugin/`) wired into the loader.
- **Operational artifacts** — `app/README.md` (full onboarding, project structure, tech stack, IPC surface, testing, build & deploy, troubleshooting); 14 SQLite migrations; Docker compose for 5-node cluster; plugin deploy script; sample K-Line plugin demonstrating the Plugin SDK end-to-end.
...
## Recently completed (2026-07-30 session — all pushed to `origin/main`, HEAD `c094cfd`)

| Commit | Spec area | Description |
|---|---|---|
| `c094cfd` | docs | Bee Client comprehensive `app/README.md` (onboarding, project structure, tech stack, IPC surface, testing, build & deploy, troubleshooting) |
| `ff1d864` | ops | Docker deployment artifacts — 5-node cluster compose + plugin deploy script + Bee Client image recipe |
| `c0bfc42` | plugins | Sample K-Line plugin (`examples/kline_plugin/`) wired into the loader + `Reload from disk` button on Datasources page |
| `3797ae9` | viz | `klinecharts` K-Line panel integration (Application Dashboard K-Line tile) |
| `7e71d70` | tests | End-to-end Bee cluster integration tests against the Rust harness |
| `ade180d` | fix | Plugin directory scan + topology multi-node verification |
| `93f132d` | viz | `@xyflow/react` cluster topology graph (symmetric ring, leader chassis, orthogonal edges, runtime overlay) |
| `f5b8356` | fix | Settings Cancel button — React hooks violation |
| `2c6a90c` | viz | Topology auto-discover + Plugins page + ECharts trend panels + DAG designer (Input → Handler(s) → Output, cross-Pipeline edges) + dynamic SchemaForm |
| `d868d0c` | ui | Visual polish — settings merge, single-row top header, dashboard editor (add/drag/resize/remove panels), topology graph wiring |
