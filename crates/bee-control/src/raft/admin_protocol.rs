...
pub enum AdminRequest {
    ...
    /// `dispatch_with_apply` handles this by
    /// submitting the op to its own Raft log.
...
pub enum AdminResponse {
    ...
    /// `RpcMessage::AdminForwardReply` carries
    /// this payload back; the follower matches
    /// the `request_id` and sends the `response`
    /// to the original CLI client. The
    /// `request_id` is opaque (a u64 generated by
    /// the follower).
